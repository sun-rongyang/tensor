...
public:
  ...
  virtual QNValPtr Minus(void) const = 0;

  virtual void AddAssign(const QNValPtr &prhs_base) = 0;
...
class NormalQNVal : public QNVal {
public:
  NormalQNVal(void) : val_(0) {}

  NormalQNVal(const long val) : val_(val) {}

  QNValPtr Clone(void) const {
    return std::make_shared<NormalQNVal>(val_);
  }
  ...
  QNValPtr Minus(void) const {
    return std::make_shared<NormalQNVal>(-val_);
  }

  void AddAssign(const QNValPtr &prhs_base) {
    auto prhs_derivate = std::static_pointer_cast<NormalQNVal>(prhs_base);  // Do a safe downcasting.
    val_ += prhs_derivate->val_; 
  }

  void Load(std::ifstream &ifs) { ifs >> val_; }

  void Dump(std::ofstream &ofs) { ofs << val_ << std::endl; }
